...
local function log_err(...) c2.log(c2.LogLevel.Critical, (current_channel_name and ("/" .. current_channel_name) or ""), ...) end

local function add_status_msg(text)
    if target_chatterino_channel then
        target_chatterino_channel:add_system_message(text)
    end
end
...
local function send_ws_message(payload_table)
    ...
    ws_connection:send_text(json_payload)
end
...
local function handle_incoming_message(payload_str)
    local pusher_msg = json.decode(payload_str)
    if type(pusher_msg) ~= "table" then log_err("Failed decode/parse incoming:", payload_str); return end
    ...
    if event == "pusher:connection_established" then
        ...
        add_status_msg("Connected to kick.com/" .. current_channel_name)
    ...
    elseif event == PUSHER_CHAT_EVENT then
        if pusher_msg.channel == ("chatrooms." .. current_chatroom_id .. ".v2") and data and type(data) == "string" then
            local msg_data = json.decode(data)
...
local function on_websocket_close()
    ...
    if current_channel_name then add_status_msg("Disconnected from kick.com/" .. current_channel_name) end
...
connect_websocket = function()
    ...
    add_status_msg("Connecting to kick.com/" .. current_channel_name .. "")
    ws_connection = c2.WebSocket.new(PUSHER_URL_BASE .. PUSHER_PARAMS, {
...
disconnect_websocket = function(called_from_error)
    ...
    is_connected = false
    if not called_from_error and was_connected and previous_name then add_status_msg("Disconnected from kick.com/" .. previous_name) end
end
...
local function fetch_kick_channel_id(channel_name_to_lookup, callback_on_success)
    add_status_msg("Looking up kick.com/" .. channel_name_to_lookup .. "")
    ...
    request:on_success(function(res)
        ...
        if status == 200 then
            local data_table = json.decode(body)
            if type(data_table)=='table' and data_table.chatroom and data_table.chatroom.id then
                callback_on_success(channel_name_to_lookup, tostring(data_table.chatroom.id))
            else
                log_err("API JSON structure error:", body); add_status_msg("API response structure error."); should_be_connected=false
            end
        else
            add_status_msg("API Error: " .. (status==404 and "Not found." or "Status "..status)); should_be_connected=false
        end
    end)
...
local function cmd_kick_handler(ctx)
    ...
    if #ctx.words >= 2 then
        ...
        if current_channel_name == requested_name and (is_connected or should_be_connected) then add_status_msg("Already on kick.com/"..requested_name..""); return end
...
c2.log(c2.LogLevel.Info, "Loading...")
